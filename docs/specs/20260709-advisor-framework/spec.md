...
> **Date:** 2026-07-09
> **Status:** implemented（P1 Claude B = 唯一 orchestrator 前置 dispatch 卡片，已實作；**P2 原生 advisor A** 已落地；**Codex/Cursor 無 orchestrator 前置 dispatch hook → 改走 AGENTS.md/CLAUDE.md advisory 指引 + 各 subject 強制注入 guide 提示**；2026-07-09 修正：移除 Codex `SubagentStart` 誤注入與卡片 2h 去重）
...
- **Codex：無 orchestrator 前置 dispatch hook（2026-07-09 修正）**：Codex **無**
  `PreToolUse(Task)`——其 `PreToolUse` 只攔 `Bash` / `apply_patch` / MCP，不攔子代理
  創建。曾一度接 **`SubagentStart` → `advisor-card.mjs`**，但該事件的 `additionalContext`
  進的是**子代理（CHILD）自己的 context**、且在**派工決策之後**才 fire——對「orchestrator
  決定 spawn 成什麼形態/模型/提示厚度」**毫無幫助**（注入對象與時機都錯）。故**移除**
  該接線。Codex 的層級指引改由**跨客戶端 advisory 指引**承載：**AGENTS.md / CLAUDE.md
  的「派工前先查層級」條款**（always-loaded rules）＋**各 subject 強制注入的 workflow
  guide 提示**（見「跨客戶端 dispatch 指引」節）。這是**指引非硬閘**——Codex 沒有能在
  spawn 前左右決策的 hook，無法做成硬性 gate。→ 原 Open question #1 就此關閉。
  卡片產生器 `advisor-card.mjs` 只保留 Claude `PreToolUse(Task|Agent)` 一條真前置注入。
...
- **Cursor：無 orchestrator 前置 dispatch hook**：Cursor hook 表面僅
  `beforeShellExecution` / `afterFileEdit` / `stop` / `beforeSubmitPrompt`
  （見 `agent-kit/hooks/clients/cursor.hooks.json`）。其 `preToolUse(Task)` 與
  `subagentStart` 皆為 **allow/deny 型**（deny 時才有 `agent_message`），**無法對
  orchestrator advisory 注入**；故**無 L1 dispatch 卡片**、亦無原生 advisor（A 不可用）。
  Cursor 的層級指引同樣改由**跨客戶端 advisory 指引**承載：**AGENTS.md / CLAUDE.md
  的「派工前先查層級」條款**＋**各 subject 強制注入的 workflow guide 提示**，並保底靠
  `model-tier-prompting` 的 skill description 召回（已在 profile）。**指引非硬閘**，未偽造
  任何 Cursor dispatch 事件。（既有 `beforeSubmitPrompt` → `prompt-skill-router.mjs`
  的 `model-tier-prompting` / `refine` intent 規則**維持不變**，非本框架的派工卡片載體。）

### 跨客戶端 dispatch 指引（Codex/Cursor 的真正 fallback）

Codex/Cursor 無法在 spawn 前把卡片注入 orchestrator，故**不做 hook、不做硬閘**，改用
**always-loaded 指引 + subject guide 提示**：

1. **harness-dev**：`AGENTS.md` 與 `CLAUDE.md`（互為鏡像、always-loaded）新增
   **「派工前先查層級（advisory）」** 條款——派子代理 / 委派 / fan-out 前先諮詢
   `model-tier-prompting` 做層級自評（依 roster Intelligence Index 排序當前可用模型、選
   最強可用者、按任務難度選子代理形態與提示厚度）；明示**非硬閘**。harness-dev **無**
   force-injected workflow-guide skill（SessionStart 只注入脫敏 advisory，見
   `hook-router.mjs`），故 always-loaded rules 即足夠，**不另建 guide 檔**。
   契約：`checks.py check_hook_wiring` 斷言兩檔含「派工前先查層級」+ `model-tier-prompting`。
2. **各 subject**（後續 propagation，本次不改 subject）：各 subject 已有 force-injected
   workflow guide，未來把同一「dispatch 前查層級」指針加進去即可（見「Open questions」下
   的 subject guide 清單）。
...
定案設計（**A + B 互補**；2026-07-09 修正 B 的跨客戶端範圍）：

- **機制 B（orchestrator 前置卡片，僅 Claude Code）**：在 Claude `PreToolUse(Task|Agent)`
  上，於 spawn 前把「任務 → 層級查表 → 建議子代理形態 + 模型 + 提示厚度」注入
  **orchestrator** 的 additionalContext，**強制把層級查表上桌**。這是**唯一**能在派工
  決策點左右 orchestrator 的真前置注入——Codex `SubagentStart`（子代理/後置）、Cursor
  `preToolUse`/`subagentStart`（allow/deny 型）**都不行**，故 B 不跨客戶端。
- **機制 A（加值，僅 Claude Code）**：疊加原生 `advisor`，在決策點提供**第二意見**。
- **跨客戶端保底（advisory，非硬閘）**：Codex/Cursor 靠 always-loaded 的
  AGENTS.md/CLAUDE.md「派工前先查層級」條款 + 各 subject 強制注入的 workflow guide 提示
  （見上節）承載——這才是 B 到不了處的真正 fallback，不是任何 hook。
...
落點（2026-07-09 修正）：**Claude Code → L1+A**（完整 A+B，唯一有 orchestrator 前置
卡片）；**Codex → L2/L3**（無前置 hook、無 A——`SubagentStart` 子代理/後置不可用）；
**Cursor → L2/L3**（無前置 hook、無 A）。此處 **L2 = always-loaded AGENTS.md/CLAUDE.md
「派工前先查層級」+ subject workflow guide 提示**（advisory，非硬閘），**非** hook。A 永遠是
**Claude Code 專屬的最上層加值**，不是任何客戶端的保底層——保底永遠由 L2/L3 承擔。

## 可行性判定（逐客戶端）

| 客戶端 | 判定 | 理由（一行） |
|---|---|---|
| **Claude Code** | **可做（can-do，A+B）** | `PreToolUse` matcher **`Task`** 可在 spawn 前把 dispatch 卡片注入 **orchestrator**（B，唯一真前置注入），再疊原生 `advisor`（A）於決策點；alias 提供「最新版」解析。可用模型由**模型自評**，harness 不需 allowlist。 |
| **Codex** | **無前置 hook → advisory 指引** | 無 `PreToolUse(Task)`；`SubagentStart` 的 context 進**子代理**且在派工**之後**（時機/對象都錯），不可用。無原生 advisor（A）。改走 AGENTS.md/CLAUDE.md「派工前先查層級」+ subject guide 提示（**指引非硬閘**）。 |
| **Cursor** | **無前置 hook → advisory 指引** | hook 表面僅 `beforeShellExecution` / `afterFileEdit` / `stop` / `beforeSubmitPrompt`；`preToolUse(Task)` / `subagentStart` 皆 allow/deny 型（無 advisory 注入）。無 A。同走 AGENTS.md/CLAUDE.md + subject guide（**指引非硬閘**）。 |

**總判定：可行，但非齊頭並進**——只有 **Claude Code** 有 orchestrator 前置 dispatch
卡片（B，硬性上桌）；**Codex/Cursor 無此 hook**，靠 always-loaded 的 AGENTS.md/CLAUDE.md
「派工前先查層級」條款 + 各 subject 強制注入的 workflow guide（advisory，非硬閘）承載。
跨客戶端一致的載體是**與工具無關的 roster + self-assess 式層級自評指引**：無論走卡片還是
指引，「當前可用模型」永遠由執行中模型自評。
...
1. ~~**Codex 子代理表面未坐實**~~ **（已關閉並修正，見「實作狀態」）**：Codex 無
   `PreToolUse(Task)`；`SubagentStart` 的 `additionalContext` 進**子代理**且在派工
   **之後**才 fire，對 orchestrator 的 spawn 決策**無用**——故**不接** advisor-card。
   Codex 改走 AGENTS.md/CLAUDE.md「派工前先查層級」+ subject guide（advisory）。
2. **Cursor dispatch 事件缺口**：`preToolUse(Task)` / `subagentStart` 為 allow/deny 型
   （非 advisory 注入）。若未來出現能對 orchestrator 注入 context 的前置事件可升級為
   L1；在此之前 Cursor 同走 AGENTS.md/CLAUDE.md + subject guide（advisory）。
2b. **subject guide propagation（待辦）**：上游 subject 多半已有 force-injected workflow
   guide——常見兩種注入形態：(a) 於 `UserPromptSubmit` + `SessionStart` 由 hook 腳本把
   guide skill 的 `SKILL.md` **全文**注入為 additionalContext；(b) 於 `SessionStart` /
   `InstructionsLoaded` 由 skill-router 腳本**依 intent 路由**注入 guide 指針。後續把
   「dispatch 前查層級」指針加進各 subject 的 guide 即可（本框架不改 subject；各 subject
   的實際 guide 路徑與注入點屬本機資訊，見交付報告，不寫入公開樹）。
...
**P3（Codex/Cursor advisory 指引）✅ 已完成（2026-07-09 修正）**：Codex/Cursor **無**
orchestrator 前置 dispatch hook（Codex `SubagentStart` 為子代理/後置、Cursor 事件為
allow/deny 型），故**不做 hook、不做硬閘**。改走 **AGENTS.md / CLAUDE.md 的「派工前先
查層級」條款**（always-loaded）＋**各 subject 強制注入的 workflow guide 提示**（見「跨
客戶端 dispatch 指引」節與 Open question #2b）。曾一度接的 `SubagentStart` → `advisor-card.mjs`
（Codex）**已移除**；卡片 2h 去重亦已移除。未偽造任何 dispatch 事件。
